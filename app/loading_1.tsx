...
export default function LoadingPage() {
  ...
  // Generate random values once using useState with initializer function
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 overflow-hidden relative">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {bubbles.map((b) => (
            <div
              key={b.id}
              className="bubble-animate absolute rounded-full bg-white"
              style={{
                width: `${b.size}px`,
                height: `${b.size}px`,
                left: `${b.left}%`,
        ...
        <div className="relative z-10 text-center max-w-md px-4">
          ...
          <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
            {particles.map((p) => (
              <div
                key={p.id}
                className="particle-animate absolute w-1 h-1 bg-white rounded-full"
                style={{
                  left: `${p.left}%`,
                  top: `${p.top}%`,
